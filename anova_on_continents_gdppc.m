%% ANOVA kontinensek szerint: GDP/fo (2010-2023)
% Csoportositas: Eszak-Amerika, Del-Amerika, Europa, Azsia, Oceania
% Egyseg: orszagonkenti idoszaki atlag GDP/fo (1 sor/orszag)
% GDP: 2011 PPP USD (Maddison)
...
country_mean_gdppc = splitapply(@mean, F.gdp_per_capita, G_country);
country_continent  = splitapply(@(x) x(1), F.continent, G_country);

country_tbl = table(country_list, country_mean_gdppc, country_continent, ...
    'VariableNames', {'country', 'mean_gdp_per_capita', 'continent'});

country_tbl = country_tbl(~isnan(country_tbl.mean_gdp_per_capita), :);

% -------------------------------------------------------------------------
% Leiro statisztika kontinensenkent
% -------------------------------------------------------------------------
fprintf('\n=== Beallitasok ===\n');
fprintf('Valtozo : GDP/fo (2011 PPP USD)\n');
fprintf('Idoszak : %d-%d\n', year_start, year_end);
...
fprintf('%-20s  %6s  %14s  %14s\n', 'Kontinens', 'N', 'Atlag (USD)', 'Std (USD)');
fprintf('%s\n', repmat('-', 1, 60));
for i = 1:n_cont
    mask = strcmp(country_tbl.continent, cont_list{i});
    vals = country_tbl.mean_gdp_per_capita(mask);
    fprintf('%-20s  %6d  %14.0f  %14.0f\n', cont_list{i}, numel(vals), mean(vals), std(vals));
end

% Csoportmeret ellenorzese
[G2, ugrps] = findgroups(categorical(country_tbl.continent));
grp_sizes   = splitapply(@numel, country_tbl.mean_gdp_per_capita, G2);
if any(grp_sizes < 2)
...
% Feltetelvizsgalat
% -------------------------------------------------------------------------
y   = country_tbl.mean_gdp_per_capita;
...
% -------------------------------------------------------------------------
figure('Name', 'GDP/fo kontinensenkent', 'NumberTitle', 'off');
boxplot(y, grp);
ylabel('Atlagos GDP/fo (2011 PPP USD)');
xlabel('Kontinens');
title(sprintf('ANOVA – GDP/fo kontinensenkent (%d–%d)', year_start, year_end));
grid on;
...
title('Tukey post-hoc: GDP/fo – kontinensek paronkenti osszehasonlitasa');
