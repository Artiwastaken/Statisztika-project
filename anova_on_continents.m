%% ANOVA kontinensek szerint: fossil_share_energy (2010-2023)
...
required_vars = {'country','iso_code','year','fossil_share_energy'};
...
has_needed = ~isnan(T.fossil_share_energy);
...
country_tbl = table(country_list, country_mean_fossil, country_continent, ...
    'VariableNames', {'country', 'mean_fossil_share_energy', 'continent'});

country_tbl = country_tbl(~isnan(country_tbl.mean_fossil_share_energy), :);
...
for i = 1:n_cont
    ...
    vals = country_tbl.mean_fossil_share_energy(mask);
...
grp_sizes   = splitapply(@numel, country_tbl.mean_fossil_share_energy, G2);
if any(grp_sizes < 2)
...
% Feltetelvizsgalat
% -------------------------------------------------------------------------
y   = country_tbl.mean_fossil_share_energy;
...
% -------------------------------------------------------------------------
figure('Name', 'Fossil share energy kontinensenkent', 'NumberTitle', 'off');
boxplot(y, grp);
ylabel('Atlagos fossil\_share\_energy (%)');
xlabel('Kontinens');
title(sprintf('ANOVA – fossil share energy kontinensenkent (%d–%d)', year_start, year_end));
grid on;
...
title('Tukey post-hoc: kontinensek paronkenti osszehasonlitasa');
